import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:firebase_auth/firebase_auth.dart';

import '../models/reminder_model.dart';

class ReminderFirestoreDataSource {
  final FirebaseFirestore firestore;
  final FirebaseAuth auth;

  const ReminderFirestoreDataSource({
    required this.firestore,
    required this.auth,
  });

  String get _uid {
    final uid = auth.currentUser?.uid;
    if (uid == null || uid.isEmpty) {
      throw Exception('User not authenticated');
    }
    return uid;
  }

  CollectionReference<Map<String, dynamic>> get _remindersCollection =>
      firestore.collection('users').doc(_uid).collection('reminders');

  Future<List<ReminderModel>> getReminders() async {
    final snapshot =
        await _remindersCollection.orderBy('updatedAt', descending: true).get();

    return snapshot.docs
        .map((doc) => ReminderModel.fromJson(doc.data()))
        .toList();
  }

  Future<ReminderModel?> getReminderById(String id) async {
    final snapshot = await _remindersCollection.doc(id).get();

    if (!snapshot.exists) return null;

    final data = snapshot.data();
    if (data == null) return null;

    return ReminderModel.fromJson(data);
  }

  Future<void> saveReminder(ReminderModel model) async {
    final now = DateTime.now();

    await _remindersCollection.doc(model.id).set({
      ...model.toJson(),
      'id': model.id,
      'patientId': _uid,
      'isSynced': true,
      'createdAt': model.createdAt.toIso8601String(),
      'updatedAt': now.toIso8601String(),
    }, SetOptions(merge: true));
  }

  Future<void> updateReminder(ReminderModel model) async {
    final now = DateTime.now();

    await _remindersCollection.doc(model.id).set({
      ...model.toJson(),
      'id': model.id,
      'patientId': _uid,
      'isSynced': true,
      'createdAt': model.createdAt.toIso8601String(),
      'updatedAt': now.toIso8601String(),
    }, SetOptions(merge: true));
  }

  Future<void> deleteReminder(String id) async {
    await _remindersCollection.doc(id).delete();
  }
}